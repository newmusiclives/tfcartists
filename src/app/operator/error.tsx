...
export default function OperatorError({
  ...
  useEffect(() => {
    logger.error("Operator error", { message: error.message, digest: error.digest });
  }, [error]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 dark:from-purple-950 via-white dark:via-zinc-900 to-indigo-50 dark:to-indigo-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white dark:bg-zinc-900 rounded-2xl shadow-xl p-8 text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 bg-purple-100 dark:bg-purple-900 rounded-full mb-6">
          <AlertCircle className="w-10 h-10 text-purple-600 dark:text-purple-300" />
        </div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Operator Dashboard Error</h1>
