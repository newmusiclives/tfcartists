    ...
    <main className="min-h-screen bg-gray-50 dark:bg-zinc-950 dark:text-zinc-100">
      <nav className="border-b bg-white dark:bg-zinc-900">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <Code className="w-6 h-6 text-amber-600 dark:text-amber-300" />
          <div>
            <h1 className="font-bold text-gray-900 dark:text-white">API Reference</h1>
            <p className="text-xs text-gray-600 dark:text-zinc-400">REST API documentation for TrueFans Radio</p>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border p-6 mb-8">
          ...
          <div className="grid sm:grid-cols-3 gap-4 text-sm">
            <div className="border rounded-lg p-3">
              ...
              <p className="text-gray-600 dark:text-zinc-400">Pass <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded text-xs">Authorization: Bearer YOUR_API_KEY</code> in the request header.</p>
            </div>
            <div className="border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-gray-600 dark:text-zinc-400">Authenticated via NextAuth session cookie. Login at <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded text-xs">/login</code> first.</p>
            </div>
            <div className="border rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                <Radio className="w-4 h-4 text-green-600 dark:text-green-300" />
          ...
          <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-lg text-sm text-amber-800 dark:text-amber-400">
            <p className="font-medium">Webhook Signatures</p>
            <p className="mt-1">All webhook deliveries include <code className="bg-amber-100 dark:bg-amber-900 px-1 rounded">X-Webhook-Signature</code> (HMAC-SHA256), <code className="bg-amber-100 dark:bg-amber-900 px-1 rounded">X-Webhook-Event</code>, and <code className="bg-amber-100 dark:bg-amber-900 px-1 rounded">X-Webhook-Timestamp</code> headers.</p>
          </div>
        </div>
              ...
              {endpoints.map((ep, i) => (
                <div key={i} className="px-5 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800">
                  <div className="flex items-center gap-3">
                    ...
                    <code className="text-sm font-mono text-gray-800 dark:text-zinc-100 flex-1">{ep.path}</code>
                    <span className={`text-xs font-medium ${AUTH_LABELS[ep.auth].color}`}>
                      {AUTH_LABELS[ep.auth].label}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600 dark:text-zinc-400 mt-1 ml-14">{ep.description}</p>
                  {ep.params && (
                  ...
                  {ep.body && (
                    <p className="text-xs text-gray-400 mt-1 ml-14">
                      Body: <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded">{ep.body}</code>
                    </p>
                  )}
                </div>
              ))}
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border p-6">
          ...
          <div className="text-sm text-gray-600 dark:text-zinc-300 space-y-2">
            ...
            <p>Exceeded limits return <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded">429 Too Many Requests</code> with a <code className="bg-gray-100 dark:bg-zinc-800 px-1 rounded">Retry-After</code> header.</p>
          </div>
        </div>
      </div>
    </main>
